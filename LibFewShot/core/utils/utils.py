...
def get_shuffle(n_way, k_shot, return_dict=False):
    ...
    if return_dict:
        return dict(zip(shuffle_idx.tolist(), torch.arange(n_way).repeat(k_shot).tolist()))
    
    return shuffle_idx

def check_gpu_memory_usage(gpu_ids):
    for id in gpu_ids:
        print("torch.cuda.memory_allocated: %fGB"%(torch.cuda.memory_allocated(id)/1024/1024/1024))
        print("torch.cuda.memory_reserved: %fGB"%(torch.cuda.memory_reserved(id)/1024/1024/1024))
        print("torch.cuda.max_memory_reserved: %fGB"%(torch.cuda.max_memory_reserved(id)/1024/1024/1024))
