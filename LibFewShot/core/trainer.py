...
class Trainer(object):
    ...
    def __init__(self, rank, config):
        ...
        if self.config["k_fold"] == 1000:
            self.config["k_fold"] = self.config['way_num'] * self.config['shot_num']
        ...
        with torch.set_grad_enabled(enable_grad):
            ...
            for batch_idx, batch in enumerate(zip(*loader)):
                ...
                if ((batch_idx + 1) * log_scale % self.config["log_interval"] == 0) or (
                    ...
                    print(info_str)
                    if len(self.config['eval_types']) == 2:
                        print("Loss_diff {:.3f}\t Acc_diff: {:.3f}".format(
                            output_dict[self.config['eval_types'][0]]['loss']-output_dict[self.config['eval_types'][1]]['loss'],
                            output_dict[self.config['eval_types'][0]]['accuracy']-output_dict[self.config['eval_types'][1]]['accuracy']))
                end = time()
    ...
    def _save_intermediate_logs(self):
        ...
        # Creating dataset direcory if it doesn't already exist
        if not os.path.isdir(result_path):
            os.mkdir(result_path)

        result_path = os.path.join(result_path, f"{self.config['way_num']}way-{self.config['shot_num']}shot")
        # Creating n-way k-shot dataset
        if not os.path.isdir(result_path):
            os.mkdir(result_path)
